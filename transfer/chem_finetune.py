...
from transfer.chem_splitters import scaffold_split, random_scaffold_split,random_split
from transfer.model  import GNN, GNN_graphpred
from transfer.chem_loader import MoleculeDataset
...
def get_args():
    ...
    parser.add_argument('--cl_exp_dir', type=str, default="pretrain_models_self")
    parser.add_argument('--cl_model_name', type=str, default="cl_model_100.pth")
    ...
    parser.add_argument('--seed', type=int, default=1304, help="Seed for splitting the dataset.")
...
if __name__ == "__main__":
    ...
    if args.split == "scaffold":
        smiles_list = pd.read_csv(os.path.join(args.dataset_root, args.dataset, 'processed/smiles.csv'), header=None)[
            0].tolist()
    ...
    args.input_model_file = os.path.join('../pretrain_models_self/model', args.cl_model_name)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        cl_loss = train(args, model, device, train_loader, optimizer)
        # print("====Evaluation")
        if args.eval_train:
        ...
        logger.info(
            "Epoch: {}，CL_Loss: {:.4f},Train Acc: {:.2f} , Val Acc: {:.2f}, Test Acc: {:.2f} ".format
            (epoch,cl_loss, train_acc * 100, val_acc * 100, test_acc * 100))

        if test_acc > best_test_acc:
            best_test_acc = test_acc


    train_std = np.array(train_acc_list).std()
    val_std = np.array(val_acc_list).std()
    test_std = np.array(test_acc_list).std()
    logger.info("Best Test Acc: {:.2f} ± {:.2f}".format(best_test_acc * 100, test_std * 100))
    ...
    with open(result_path, 'a+') as f:
        f.write(args.dataset + ' ' + str(args.seed) + ' ' + str(best_test_acc)+'±'+str(test_std))
        f.write('\n')