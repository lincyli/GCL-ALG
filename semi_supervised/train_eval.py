...
from semi_supervised.gca import get_gca_param, get_gca_model, train_gca
from semi_supervised.gca import GCA_Classifier, train_gca_cls, eval_gca_acc

sys.path.append(os.path.abspath(os.path.join('..')))

from view_generator import ViewGenerator, GIN_NodeWeightEncoder, mlp_edge_encoder, GIN_Classifier
...
def benchmark_exp(device, logger, dataset, model_func,
    ...
    for fold, (train_idx, test_idx, val_idx, semi_idx) in enumerate(
        ...
        for epoch in range(1, epochs + 1):
            ...
            train_acc = eval_f1(model, semi_loader, device, with_eval_mode)
            test_acc = eval_f1(model, test_loader, device, with_eval_mode)
...
def joint_cl_exp(
    ...
    for fold, (train_idx, test_idx, val_idx, semi_idx) in enumerate(
        ...
        logger.info("*" * 10)
        logger.info("Fold: %d" % fold)
        train_view_losses, cl_losses, cls_losses, sim_losses = [], [], [], []
        train_dataset = dataset[train_idx]
        test_dataset = dataset[test_idx]
        val_dataset = dataset[val_idx]
        semi_dataset = dataset[semi_idx]
        ...
        model = model_func(dataset)
        model.to(device)
        view_gen1 = ViewGenerator(dataset, 128,128,GIN_NodeWeightEncoder ,mlp_edge_encoder, add_mask)
        view_gen2 = ViewGenerator(dataset, 128,128,GIN_NodeWeightEncoder, mlp_edge_encoder,add_mask)
        ...
        for epoch in range(1, epochs + 1):
            ...
            train_view_losses.append(train_view_loss)
...
def train_node_weight_view_gen_and_cls(view_gen1, view_gen2, view_optimizer,
    ...
    for data in loader:
        ...
        loss = (sim_loss + cls_loss + cl_loss) / accumlation_step
        loss.backward()
...
@torch.no_grad()
def eval_f1(classfier, loader, device, with_eval_mode=True):
    if with_eval_mode:
        classfier.eval()
    ...
    with torch.no_grad():
        for data in loader:
            data = data.to(device)
            # outputs = classfier.get_embedding(data)
            outputs = classfier(data)
